...
;*                                                                                                 *
;* This program navigates the eebot through a maze. The eebots sensors are used to detect obstacles* 
;* and help it stay on a path created by black tape. This program uses a state approach as the     *
;* eebot changes it movement (state) based on an event, such as an intersection or obstacle (input)*
;*                                                                                                 *
;* Group Members: Maheem Q., Ashviny Y., Aananth K.                                                *
;***************************************************************************************************
...
; ---------------------------
LINE_VARIANCE       	FCB   $12       	; Variance from testing: High - [(High + Low)/2]
BOW_VARIANCE        	FCB   $4D       	; Establish baseline for sensors
PORT_VARIANCE       	FCB   $1A                	 
MID_VARIANCE        	FCB   $46
STARBOARD_VARIANCE  	FCB   $33
...
DISPATCHER    	JSR   CFRM_START                    	; Start of the Dispatcher
              	RTS

CFRM_START    	CMPA  #START                          ; Confirm if the robot's state is START
              	BNE   CFRM_FORWARD                  	; If not, move to FORWARD state validation
              	JSR   START_ST                        ; Go to START state
              	RTS                                    	 

CFRM_FORWARD  	CMPA  #FWD                            ; Confirm if the robot's state is FORWARD
              	BNE   CFRM_STOP                     	; If not, move to ALL_STOP state validation
              	JSR   FWD_ST                          ; Go to FORWARD state
              	RTS
             	 
CFRM_REV_TRN	  CMPA  #REV_TRN                        ; Confirm if the robot's state is REV_TURN
              	BNE   CFRM_LEFT_FIX               	  ; If not, move to LEFT_FIX state validation
              	JSR   REV_TRN_ST                     	; Go to REV_TURN state
              	RTS                                      	 

CFRM_STOP     	CMPA  #ALL_STOP                       ; Confirm if the robot's state is ALL_STOP
              	BNE   CFRM_LEFT_TRN                 	; If not, move to LEFT_TURN state validation  
              	JSR   ALL_STOP_ST                    	; Go to ALL_STOP state  
              	RTS                                   ; *** eebot looks to turn left before turning right *** 	 

CFRM_LEFT_TRN   CMPA  #LEFT_TRN                      	; Confirm if the robot's state is LEFT_TURN
              	BNE   CFRM_RIGHT_TRN                	; If not, move to RIGHT_TURN state validation
              	JSR   LEFT                           	; Go to LEFT_TURN state
              	RTS                                                                                                                 	 

CFRM_LEFT_FIX   CMPA  #LEFT_FIX                     	; Confirm if the robot's state is LEFT_FIX
              	BNE   CFRM_RIGHT_FIX            	    ; If not, move to RIGHT_FIX state validation
              	JSR   LEFT_FIX_DONE                 	; Go to LEFT_FIX state
              	RTS

CFRM_RIGHT_TRN  CMPA  #RIGHT_TRN                      ; Confirm if the robot's state is RIGHT_TURN
              	BNE   CFRM_REV_TRN                  	; If not, move to REV_TURN state validation
              	JSR   RIGHT                          	; Go to RIGHT_TURN state                                 	 

CFRM_RIGHT_FIX  CMPA  #RIGHT_FIX                   	  ; Confirm if the robot's state is RIGHT_FIX
              	JSR   RIGHT_FIX_DONE               	  ; Go to RIGHT_FIX state
              	RTS                                  	
...
FWD_ST        	BRSET PORTAD0, $04, NO_FWD_BUMP       	; Checks if bow bumper is hit                      	 
              	...
              	JSR 	INIT_REV                                                           	 
              	LDY 	#6000                             ;6000 * 50us = 0.006s or 6ms                                 	 
              	JSR 	del_50us                                                           	 
              	JSR 	INIT_RIGHT                                                         	 
              	LDY 	#6000                                                              	 
              	JSR 	del_50us                                                           	 
...
NO_FWD_BUMP   	BRSET PORTAD0, $04, NO_FWD_REAR_BUMP  	; Checks if the stern bumper
              	...
              	JSR 	INIT_STOP                       	
              	LBRA	EXIT
             	                                          ; Until stern bumper is hit, robot is operating
NO_FWD_REAR_BUMP  LDAA	SENSOR_BOW                                                         	 
                	ADDA	BOW_VARIANCE                                                          	 
                	CMPA	BASE_BOW                        ; if sensor bow + bow variance is not equal to base bow                                  	 
                	BPL 	NOT_FIXED                       ; the eebot is disoriented (must be alligned/fixed)
...
NOT_FIXED   	  LDAA	SENSOR_PORT                       ; if eebot is disoriented to the right                               	 
              	ADDA	PORT_VARIANCE                                                          	 
              	CMPA	BASE_PORT                         ; compare sensor port + port variance to base port                                	 
              	BPL 	PART_LEFT_TRN                     ; if there is a positive difference btwn them, adjust to the left                              	 
              	BMI 	NO_PORT                                                        	 

NO_PORT       	LDAA	SENSOR_BOW                        ; if eebot is not disoriented to the left                                	 
              	ADDA	BOW_VARIANCE                                                            	 
              	CMPA	BASE_BOW                          ; check if it is oriented to perfectly in line with path                                 	 
              	BPL 	EXIT                                                               	 
              	BMI 	NO_BOW                                                         	 

NO_BOW        	LDAA	SENSOR_STBD                       ; if eebot is disoriented to the right                                  	 
              	ADDA	STARBOARD_VARIANCE                                                          	 
              	CMPA	BASE_STBD                                                          	 
              	BPL 	PART_RIGHT_TRN                    ; adjust to the left                                	 
              	BMI 	EXIT

;***************************************************************************************************

PART_LEFT_TRN     LDY 	#6000                                                           	 
              	  jsr 	del_50us                                                           	 
                	JSR 	INIT_LEFT                                                          	 
...
CHECK_LEFT_FIX    JSR 	INIT_LEFT                                                          	 
                	MOVB	#LEFT_FIX, CRNT_STATE                                            	 
                	BRA 	EXIT

;***************************************************************************************************

PART_RIGHT_TRN    LDY 	#6000                                                             	 
                	jsr 	del_50us                                                           	 
                	JSR 	INIT_RIGHT                                                         	 
...
CHECK_RIGHT_FIX   JSR 	INIT_RIGHT                                                         	 
                	MOVB	#RIGHT_FIX, CRNT_STATE                                           	 
                	BRA 	EXIT                                                                                                                                                    	 
...
LEFT_FIX_DONE   MOVB	#FWD, CRNT_STATE                                                   	 
              	JSR 	INIT_FWD                                                           	 
              	BRA 	EXIT                                                               	 
...
RIGHT_FIX_DONE  MOVB	#FWD, CRNT_STATE                                                   	 
              	JSR 	INIT_FWD                                                           	 
              	BRA 	EXIT                                                               	 
...
; Initialization Subroutines  (From Lab 5)
;***************************************************************************************************
...
;***************************************************************************************************
              	...
              	ORG 	$FFDE
              	DC.W	TOF_ISR ; Timer Overflow Interrupt VectorF
